...
def sign_url(format, cdn_resource_hostname, path, secret_key, scheme, expiry_timestamp, client_ip):
    ...
    Example:
        ...
        python3 UrlSigning3.py -r example.com -p images/photo.png -k 123abc -e 1546300800 -i 1.3.2.2 -s http -f path
        Querystring:
    ...
    token = ''.join(["secure=",
                     base64.encodestring(md5(token_key.encode()).digest()).decode().replace("+", "-").replace("/", "_").replace(
                         "=", "").split("\n")[0]])
